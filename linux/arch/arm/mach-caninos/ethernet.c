...
static int ethernet_dma_init(void)
{
    ...
    ethernet_tx_buf = dma_alloc_coherent(dev, sizeof(struct ethernet_buffer_desc) * TX_RING_SIZE, &ethernet_tx_buf_paddr, GFP_KERNEL);
    
    if (!ethernet_tx_buf) {
        return -ENOMEM;
    }
    
    ethernet_rx_buf = dma_alloc_coherent(dev, sizeof(struct ethernet_buffer_desc) * RX_RING_SIZE, &ethernet_rx_buf_paddr, GFP_KERNEL);
    
    if (!ethernet_rx_buf)
    {
        dma_free_coherent(dev, sizeof(struct ethernet_buffer_desc) * TX_RING_SIZE, ethernet_tx_buf, ethernet_tx_buf_paddr);
        return -ENOMEM;
    }
...
static int ethernet_tx_buffer_alloc(void)
{
    ...
    ethernet_tx_buf = dma_alloc_coherent(dev, total, paddr, GFP_NOWAIT);
    if (!ethernet_tx_buf)
    {
        ethernet_tx_buf = NULL;
        *paddr = (dma_addr_t)(0);
...
static int ethernet_rx_buffer_alloc(void)
{
	...
	if (ret){
        ERR_MSG("bad coherent mask\n");
		return ret;
    }
    
    ethernet_rx_buf = dma_alloc_coherent(dev, total, &ethernet_rx_buf_paddr, GFP_NOWAIT);
    if (!ethernet_rx_buf)
    {
        INFO_MSG("error to alloc rx_buf");
        ethernet_rx_buf = NULL;
        *paddr = (dma_addr_t)(0);
        return -ENOMEM;
    }
