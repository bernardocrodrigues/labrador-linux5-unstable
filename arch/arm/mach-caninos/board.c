...
extern volatile int caninos_pen_release = -1;
...
{
	caninos_pen_release = val;
	smp_wmb();
	__cpuc_flush_dcache_area((void *)&caninos_pen_release, sizeof(caninos_pen_release));
	outer_clean_range(__pa(&caninos_pen_release), __pa(&caninos_pen_release + 1));
}
...
int __init board_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
	...
	return caninos_pen_release != -1 ? -ENOSYS : 0;
}
...
static int pmic_reg_setbits(u8 reg, u16 mask, u16 val)
{
    u16 tmp, orig;
	int ret;
...
#define SLOW_POWERUP 	(0x1 << 12)
#define	FAST_POWERUP	(0)
#define BIG_BIAS		(0x1 << 11)
#define SMALL_BIAS		(0)
#define LDO_EN			(0x1)

#define LDO7_VOL_1V5	(0x0)
#define LDO7_VOL_1V6	(0x1 << 13)
#define LDO7_VOL_1V7	(0x2 << 13)
#define LDO7_VOL_1V8	(0x3 << 13)
#define LDO7_VOL_1V9	(0x4 << 13)
#define LDO7_VOL_2V0	(0x5 << 13)

#define LDO1_VOL_2V6	(0x0)
#define LDO1_VOL_2V7	(0x1 << 13)
#define LDO1_VOL_2V8	(0x2 << 13)
#define LDO1_VOL_2V9	(0x3 << 13)
#define LDO1_VOL_3V0	(0x4 << 13)
#define LDO1_VOL_3V1	(0x5 << 13)
#define LDO1_VOL_3V2	(0x6 << 13)
#define LDO1_VOL_3V3	(0x7 << 13)

/*	@ldo_addr: the ldo address
	@bias:	bias select, SMALL_BIAS or BIG BIAS current
	@soft_startup:	FAST_POWERUP or SLOW_POWERUP
	@ldo_vol:	the voltage, LDOX_VOL_Y, where X is the ldo number and Y is the voltage wish
	@ldo_en:	LDO_EN or ~LDO_EN
 */
static int ldo_regulator_config(unsigned int ldo_addr, unsigned int bias, unsigned int soft_startup, unsigned int ldo_vol, unsigned int ldo_en){
	int ret;
	u16 reg_val;

	ret = pmic_reg_read(ldo_addr, &reg_val);
	if(ret){
		return ret;
	}
	...
	if(soft_startup == SLOW_POWERUP)
		reg_val |= SLOW_POWERUP;

	reg_val |= ldo_vol;

	ret = pmic_reg_write(ldo_addr, reg_val);
...
static int __init pmic_setup(void)
{
	...
	//configuring LDO1 to 2V8 for camera
	ret = ldo_regulator_config(ATC2603C_PMU_LDO1_CTL, SMALL_BIAS, FAST_POWERUP, LDO1_VOL_2V8, LDO_EN);
	if(ret){
		pr_err("fail to config LDO01");
	}
	//configuring LDO7 to 1V8 for camera
    ret = ldo_regulator_config(ATC2603C_PMU_LDO7_CTL, SMALL_BIAS, FAST_POWERUP, LDO7_VOL_1V8, LDO_EN);
	if(ret){
		pr_err("fail to config LDO01");
	}
