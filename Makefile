...
config:
	...
	$(Q)$(MAKE) -C $(CURDIR)/linux O=$(CURDIR)/build CROSS_COMPILE=arm-linux-gnueabihf- ARCH=arm caninos_k5_defconfig
...
kernel:
	...
	$(Q)cp $(CURDIR)/build/arch/arm/boot/dts/caninos-k5.dtb $(CURDIR)/output/kernel.dtb
